...
Current version: **1.1.2**. See the [changelog](CHANGELOG.md) for release notes.
...
From the repository root, run this command and replace `1.1.2` with the
version that you want to build:

```sh
./scripts/release/build-app.sh --swift-build --unsigned-dry-run \
  --skip-source-validation --version 1.1.2 --tag v1.1.2 \
  --output-dir dist/1.1.2
```

The command creates `dist/1.1.2/Syrinx-1.1.2.zip`. The archive contains the
