...
struct Run: ParsableCommand {
    ...
    @Option(name: .customLong("syrinx-url"), help: "Syrinx loopback service URL. Overrides --parakeet-url.")
    var syrinxURL: String?
    ...
    func run() throws {
        ...
        let serviceURL = syrinxURL ?? parakeetURL
        let environment = ProcessInfo.processInfo.environment
        let transcriber = try TranscriberFactory.make(
            model: chosenModel,
            parakeetURL: serviceURL,
            parakeetAPIKey: environment["PARROT_SYRINX_API_KEY"] ?? environment["PARROT_PARAKEET_API_KEY"]
        )
